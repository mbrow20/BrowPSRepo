...
SimulationWithCovMat=function(CovMatrixWt,data1,Sims,mu_beta){
  ...
  #mu_beta<<-c(1.1,.02,-1,2,1.5,0.04,1.2,-1.3,0.8,-0.5,-1.4,0.3,-0.3,0.75,1.6,-0.98)
  K<<-length(mu_beta)-2
  ...
  print(W2)
  options(warn=0)
  write.table(TFrame, file="TFrameTrimmed500.csv", col.names = T, row.names = F, sep=",", append=F)
  fileConn1<-file("BiasSumm1Trimmed.txt")
  fileConn2<-file("BiasSumm2Trimmed.txt")
  fileConn3<-file("BiasSumm3Trimmed.txt")
  writeLines(W, fileConn1)
  writeLines(W1, fileConn2)
  writeLines(W2, fileConn3)
}
