...
SimulationWithCovMat=function(CovMatrixWt,data1,Sims,mu_beta){
  ...
  dpart<<-createDataPartition(data1$treat,p=0.3,list=F)
  data2<<-data1[dpart,]
  #data2<<-data1
  ...
  m.outThree<<-m.out3
  newdata<<-test_data3[m.outThree$match.matrix,]
  
  #m.outThree$treat<<-m.out3$treat
  #test_datam<-m.outThree$X[m.outThree$match.matrix,]
  ##Sensitivity Analysis
  test_datam<<-as.data.frame(newdata)
  #tp<-ncol(test_datam)
  #colnames(test_datam)[tp]<<-'treat'
  sen_control<<-test_datam
  sen_treat<<-subset(test_data3, test_data3$treat==1)
  lmSen_cont<<-lm(formula4, data=sen_control)
  lmSen_treat<<-lm(formula4, data=sen_treat)
  sp_square <<-((nrow(sen_control)-1)*(summary(lmSen_cont)$coefficients[[31]])^2+
                 (nrow(sen_treat)-1)*(summary(lmSen_treat)$coefficients[[31]])^2)/(nrow(sen_control)+nrow(sen_treat)-2)
  sp<<-sqrt(sp_square)
  independ_ttest<<-(summary(lmSen_cont)$coefficients[[1]]-summary(lmSen_treat)$coefficients[[1]])/sp
  df_t<<-summary(lmSen_cont)$df[2]
  #for (i in 1:Sims){
    #if(i >= 1 & i <= Sims) {
     # counter = i
      #print(paste0(counter,"sensitivity"))
    #}
    #assign(paste0("sen_control",i),subset(get(paste0("test_data",i)),get(paste0("test_data",i))[,3]==0))
    #assign(paste0("sen_treat",i),subset(get(paste0("test_data",i)),get(paste0("test_data",i))[,3]==1))
    #assign(paste0("lmSen_control",i), lm(formula4, data=get(paste0("sen_control",i))))
    #assign(paste0("lmSen_treat",i), lm(formula4, data=get(paste0("sen_treat",i))))
    #assign(paste0("sp_square",i), ((nrow(get(paste0("sen_control",idata8_control)-1)*(summary(lm17_control)$coefficients[[31]])^2+
...
#sp<-sqrt(sp_square)"))
  ...
  print(W2)
  W3<-sprintf("The t-test for the sensitivity analysis for matched units is t(%d)= %.2f",df_t,independ_ttest)
  print(W3)
}
